...
import styles from './AnimePageItem.module.scss';
...
function AnimePageItem() {
    const isMovieBanner = dataItems.filter((item) => item?.isBanner?.isAnime === true);
    const isMovieGenresList = videoGenresList.filter((item) => item.valueGenresType.includes('anime'));
    const movieGenres = { ...isMovieGenresList[0] };

    // console.log('isMovieGenresList', isMovieGenresList);
